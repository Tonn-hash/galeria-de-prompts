...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(({ className, variant, items, ...props }, ref) => (
  <nav
    ref={ref}
    className={cn(sidebarVariants({ variant }), "flex space-x-2 lg:flex-col lg:space-x-0 lg:space-y-1", className)}
    {...props}
  >
    {items.map((item) => (
...
const SidebarTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("font-semibold text-lg tracking-tight", className)} {...props} />
  ),
)
